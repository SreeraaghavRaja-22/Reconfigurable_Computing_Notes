...
module timer #(parameter int WIDTH=32)	   		  
	...
	IDLE : begin
        ...
        if (go == 1'b1) begin
            ...
            // Solution: set next_count = cycles
            // next_count  = 1;
    ...
    // Solution: count down unitl reaching cycles_r
	WORKING : begin
        done = 1'b0;

        // pre-optimization -- be careful with this 
        next_count = count_r - 1'b1; 
	   
	    if (count_r == WIDTH'(1)) begin // initially setting count_r == cycles_r
            done       = 1'b1;	     
            next_state = IDLE;
	    end	
